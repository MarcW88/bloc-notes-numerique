...
- Contenu utile hors navigation et hors section Sources : **1324 mots**.
- H2 substantiels : **7**.
- Tableaux : **2**.
...
- `Commencez par décrire une journée réelle d’utilisation` — 164 mots ; 3 paragraphe(s) ; 0 tableau(x) ; 0 liste(s).
- `Les critères éliminatoires passent avant le confort` — 227 mots ; 2 paragraphe(s) ; 1 tableau(x) ; 0 liste(s).
- `À chaque priorité correspond une famille d’appareils à examiner` — 174 mots ; 2 paragraphe(s) ; 1 tableau(x) ; 0 liste(s).
- `Les documents et l’export sont souvent plus importants que le stylet` — 167 mots ; 3 paragraphe(s) ; 0 tableau(x) ; 0 liste(s).
- `Taille, couleur, éclairage et stylet : les critères de confort viennent ensuite` — 164 mots ; 3 paragraphe(s) ; 0 tableau(x) ; 0 liste(s).
- `Calculez le coût de la configuration réellement utilisable` — 145 mots ; 3 paragraphe(s) ; 0 tableau(x) ; 0 liste(s).
- `La checklist finale avant de passer au comparatif` — 143 mots ; 2 paragraphe(s) ; 0 tableau(x) ; 1 liste(s).

## 2. Maillage interne

- Liens internes contextuels : **9**.
- Cibles uniques : **9**.
