...
```yaml
url: /guides/ecosysteme-ouvert-ou-ferme/
...
- Contenu utile hors navigation et hors section Sources : **934 mots**.
- H2 substantiels : **7**.
- Tableaux : **1**.
...
- `« Fermé » ne veut pas dire « données prisonnières » et « ouvert » ne veut pas dire « tout fonctionne bien »` — 100 mots ; 3 paragraphe(s) ; 0 tableau(x) ; 0 liste(s).
- `Les compromis principaux entre système spécialisé et système ouvert` — 128 mots ; 2 paragraphe(s) ; 1 tableau(x) ; 0 liste(s).
- `reMarkable et Supernote : spécialisation avec des sorties vers l’extérieur` — 117 mots ; 3 paragraphe(s) ; 0 tableau(x) ; 0 liste(s).
- `BOOX : l’ouverture Android apporte des applications, avec une couche E Ink à gérer` — 112 mots ; 3 paragraphe(s) ; 0 tableau(x) ; 0 liste(s).
- `Kindle et Kobo : des écosystèmes fermés autour de la lecture, mais de plus en plus connectés` — 106 mots ; 3 paragraphe(s) ; 0 tableau(x) ; 0 liste(s).
- `L’ouverture modifie aussi la surface de gestion et de sécurité` — 116 mots ; 3 paragraphe(s) ; 0 tableau(x) ; 0 liste(s).
- `Le test pour savoir combien d’ouverture vous avez vraiment besoin` — 94 mots ; 2 paragraphe(s) ; 0 tableau(x) ; 0 liste(s).
