...
```yaml
url: /guides/formats-fichiers-compatibles/
...
- Contenu utile hors navigation et hors section Sources : **967 mots**.
- H2 substantiels : **7**.
- Tableaux : **2**.
...
- `Un format peut être « compatible » à quatre niveaux différents` — 135 mots ; 2 paragraphe(s) ; 1 tableau(x) ; 0 liste(s).
- `PDF : le format le plus courant, mais pas toujours le plus simple` — 126 mots ; 3 paragraphe(s) ; 0 tableau(x) ; 0 liste(s).
- `EPUB et DRM : le fichier n’est pas toujours librement transférable` — 114 mots ; 3 paragraphe(s) ; 0 tableau(x) ; 0 liste(s).
- `DOCX, PPTX et fichiers bureautiques : ouverture ne veut pas dire édition complète` — 127 mots ; 3 paragraphe(s) ; 0 tableau(x) ; 0 liste(s).
- `Les formats de notes propriétaires nécessitent un plan de sortie` — 106 mots ; 3 paragraphe(s) ; 0 tableau(x) ; 0 liste(s).
- `Repères documentés selon les écosystèmes` — 161 mots ; 2 paragraphe(s) ; 1 tableau(x) ; 0 liste(s).
- `La checklist compatibilité avant achat` — 71 mots ; 2 paragraphe(s) ; 0 tableau(x) ; 0 liste(s).
