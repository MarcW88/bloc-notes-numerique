...
page_role_in_standard: reference
```

## 1. Profondeur sémantique mesurable

- Contenu utile hors navigation et hors section Sources : **1053 mots**.
- H2 substantiels : **7**.
- Tableaux : **2**.
...
- `Combien coûte réellement un bloc-notes numérique ?` — 242 mots ; 2 paragraphe(s) ; 1 tableau(x) ; 0 liste(s).
- `Les postes à intégrer dans votre budget` — 170 mots ; 2 paragraphe(s) ; 1 tableau(x) ; 0 liste(s).
- `Comparez des configurations équivalentes` — 147 mots ; 2 paragraphe(s) ; 0 tableau(x) ; 1 liste(s).
- `Prix d’achat ou coût sur plusieurs années ?` — 114 mots ; 2 paragraphe(s) ; 0 tableau(x) ; 0 liste(s).
- `Quand vaut-il la peine de payer plus cher ?` — 95 mots ; 2 paragraphe(s) ; 0 tableau(x) ; 0 liste(s).
- `Économiser sans déplacer le problème` — 107 mots ; 2 paragraphe(s) ; 0 tableau(x) ; 0 liste(s).
- `Quel comparatif consulter selon votre budget ?` — 79 mots ; 2 paragraphe(s) ; 0 tableau(x) ; 0 liste(s).
