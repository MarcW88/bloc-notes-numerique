...
No third-party Python dependency is required. The script validates structured
image requests, submits only approved low-risk requests to BFL, downloads the
result into the repository, inserts the corresponding figure, and updates the
request status.
"""
...
def figure_markup(req: dict[str, Any]) -> str:
    ...
    if caption:
        lines.append(
            '  <figcaption style="margin-top:8px;font-size:.78rem;line-height:1.5;color:var(--color-text-muted);">'
...
def download_binary(url: str, timeout: int = 120) -> bytes:
    request = urllib.request.Request(url, headers={"User-Agent": "bloc-notes-numeriques-image-bot/1.0"})
    try:
        with urllib.request.urlopen(request, timeout=timeout) as response:
            data = response.read()
    except urllib.error.URLError as exc:
